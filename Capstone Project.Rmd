...
5-gram:
```{r, cache = TRUE}
sample5gram <- ngramsetfreq(ngram = 5, dataset = sampletrain) %>%
        mutate(., ngram = 5)
```

Total n-gram set:
```{r, cache = TRUE}
samplengram <- bind_rows(sample2gram, sample3gram, sample4gram, sample5gram)
```
...
#### Function that, given a string, determines if there are n-grams available from which to predict the next word
...
```{r}
ngramdisp <- function(string, dataset) {
        n <- ngramdim(string)
        stringsearch <- paste("^", string, "\\b", sep="")
        disp <- stringsearch %in% dataset[[1]]
        return(disp)
}
```

As an example, if applied with the unexisting 4-gram "and a case of":
```{r}
ngramdisp(string = "and a case of", dataset = samplengram)
```

#### Function that, given a string, selects the n-grams from which to predict the next word

INTEGRAR CON FUNCIÓN ANTERIOR, MÁS UN LOOP (REPEAT O WHILE) QUE CALIBRE N (RESTANDOLE DE A UNA UNIDAD) HASTA QUE SE ENCUENTRE UN N-GRAMA PARA PROPONER:
```{r}
ngrampred <- function(string, dataset) {
        n <- ngramdim(string)
        disp <- ngramdisp(string, dataset)
        while(disp == FALSE) {
                n <- n-1
                pred <- word(string, )
...
```{r}
head(ngrampred(string = "out of the year", dataset = samplengram), 3)
```
...
As an example, if applied again with 2-gram string "on the":
```{r}
wordpred(string = "on the", dataset = samplengram)
```
