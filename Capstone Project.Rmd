...
The following function splits each vector element (a string) into pargraphs with which it creates extra vector elements.  It also filters all "" elements.
```{r}
splitsentences <- function(dataset) {
        result <- str_split(dataset, "(?<!(mr|mrs|sr|Mr|Mrs|Sr|MR|MRS|SR))\\.|\\?|\\!|\\:") %>%
                unlist(.) %>%
                subset(., . != "")
        return(result)
}
...
The following function nests the previous one, converting its output into a vector with the strings turned into lower-case and space-separated words:
```{r}
