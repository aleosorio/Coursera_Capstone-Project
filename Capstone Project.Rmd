...
In case there is no string available in the dataset, it reduces the string's n-gram by 1 and repeats until it finds the first available string.
```{r}
stringsure <- function(string, dataset) {
        strlength <- str_count(string, boundary("word"))
        strdisp <- paste("(^|\\s)", string, "\\s\\w+", sep = "") %>%
                str_detect(dataset, .) %>%
                sum()
        while(strdisp == 0) {
        ...
        strwhere <- str_which(dataset, paste("(^|\\s)", string, "\\s\\w+", sep = ""))
        result <- stringset(string, dataset[strwhere])
        return(result)
}
```
