...
It starts by converting the input string to the same testing dataset format (lower_case_space_separated_no_punctuation_words) and reducing the string's n-gram (by removing first words) down to the "strlength" parameter (if applies).  In case there's no match for the string in the dataset, it continues reducing the string's n-gram by 1 (by taking out its first word) and searching until it finds the first available string. If it goes down to a 1-gram string and finds no result, it returns an empty dataframe (with the same format of a non-empty output).
```{r}
stringsure <- function(string, strlength, dataset) {
        strsearch <- str_to_lower(string) %>%
                str_extract_all(., boundary("word")) %>%
                unlist(., use.names = FALSE) %>%
                str_flatten(., collapse = " ") %>%
                if_else(str_count(., boundary("word")) > strlength, word(., str_count(., boundary("word")) - strlength + 1, -1), .)
...
```{r}
wordpred2 <- function(string, strlength, dataset) {
        pred <- stringsure(string, strlength, dataset)
        if(nrow(pred) >0) {
...
#### Function that returns the predicted word, with initial search within the pre-calculated popular n-grams dataset (wordpredgrams)
If there is no match within the pre-calculated n-grams dataset, a prediction is conducted using the wordpred2 function, with a 2% sample of the testdata.
```{r}
wordpredfinal <- function(string, strlength, precision) {
        strpos <- which(string == wordpredgrams$string)
        ...
        if(lenstrpos > 0) {
                ...
                result <- wordpred2(string, strlength, dataset)
        }
        return(result)
}
```
