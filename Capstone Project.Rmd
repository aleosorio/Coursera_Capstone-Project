...
The following function turns strings into lower-case format and splits them into pargraphs (using ".", ":" type characters and ommiting words such as "sr.", "mr.", etc.).  Each paragraph is treated as an extra vector element.  It also filters all "" elements.
```{r}
splitsentences <- function(dataset) {
        result <- str_to_lower(dataset) %>%
                str_split(., "(?<!(mr|mrs|sr|st|att))\\.|\\?|\\!|\\:") %>%
...
The following function nests the previous one, converting its output into a vector with the strings turned into one-space-separated words:
```{r}
tidystrings <- function(dataset) {
        result <- splitsentences(dataset)
        datalength <- length(result)
        result <- result %>%
...
```{r, cache = TRUE}
words_twitter <- tidystrings(datatwitter)
words_blogs <- tidystrings(datablogs)
words_news <- tidystrings(datanews)
```
...
PENDIENTE:
1. PROBAR TRUNCANDO TEXTOS CUANDO APARECE ALGUN ". : ? !" (OMITIR MR. MRS. SR.).
2. VARIABILIZAR ESPACIO MUESTRAL EN FUNCIÓN DE N-GRAMS A BUSCAR: BUSCAR MNEMOTECNICA PARA DEFINIR N A PARTIR DEL % DE CADA SEGMENTO N-GRAM (PARETO 80/20)
  A) 1-GRAMS: SEGMENTAR EN PALABRAS MÁS POPULARES (MENOR ESPACIO MUESTRAL -0.001-) Y MENOS POPULARES (EJ: 0.05)
  B) 2-GRAMS (EJ: 0.1)
  C) 3-GRAMS (EJ:0.2)
