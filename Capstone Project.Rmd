...
In case there is no n-gram available, it reduces n by 1 and repeats until it finds the best available n-gram set.
```{r}
ngrampred <- function(string, dataset) {
        ...
        ngramset <- dataset %>%
                filter(., ngram == (numwords + 1) & str_detect(string, stringsearch))
        return(ngramset)
}
```
