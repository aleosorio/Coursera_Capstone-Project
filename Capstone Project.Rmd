...
Function that nested the previous function, within a loop along the length of the dataset.  The returned dataset (twogramfreq) was grouped by words, summarized and arranged by frequency in order to easily visualize the most frequent two_grams within each dataset.

```{r}
twogramsetfreq <- function(dataset) {
        setfreq <- data_frame(word = character(), freq = numeric())
        for(i in seq_along(dataset)) {
               setfreq <- bind_rows(setfreq, twogramrowfreq(dataset, rownum = i))
        }
        setfreq <- setfreq %>%
                group_by(word) %>%
                summarize (., freq = sum(freq)) %>%
...
```{r, echo = FALSE}
head(twogramsetfreq(dataset = words_news[1:5]), 5)
```

#### Single row 3-grams generator

Function that returns a list of 3-grams per row, using the same mechanics of the 2-grams generator.
...
Function that uses the same mechanics of the single row 2-grams frequency counter, applied to the 3-grams generator.
...
#### Multirow 3-grams frequency counter

Function that nested the previous function, within a loop along the length of the dataset.  The returned dataset (threegramfreq) was grouped by words, summarized and arranged by frequency in order to easily visualize the most frequent two_grams within each dataset.

```{r}
threegramsetfreq <- function(dataset) {
        setfreq <- data_frame(word = character(), freq = numeric())
        for(i in seq_along(dataset)) {
               setfreq <- bind_rows(setfreq, threegramrowfreq(dataset, rownum = i))
        }
        setfreq <- setfreq %>%
                group_by(word) %>%
                summarize (., freq = sum(freq)) %>%
...
```{r, echo = FALSE}
head(threegramsetfreq(dataset = words_news[1:5]), 5)
```
...
Likewise, when counting two-grams frequencies:

```{r, cache = TRUE}
freqtwograms_twitter <- twogramsetfreq(dataset = words_twitter[1:1000])
head(freqtwograms_twitter)
```
...
Finally, when counting three-grams frequencies:

```{r, cache = TRUE}
freqthreegrams_twitter <- threegramsetfreq(dataset = words_twitter[1:1000])
head(freqthreegrams_twitter)
```
...
The following word frequency results were obtained.
...
```{r, cache = TRUE}
freqtwograms_blogs <- twogramsetfreq(dataset = words_blogs[1:1000])
head(freqtwograms_blogs)
```
...
```{r, cache = TRUE}
freqthreegrams_blogs <- threegramsetfreq(dataset = words_blogs[1:1000])
head(freqthreegrams_blogs)
```
...
```{r, cache = TRUE}
freqtwograms_news <- twogramsetfreq(dataset = words_news[1:1000])
head(freqtwograms_news)
```
...
```{r, cache = TRUE}
freqthreegrams_news <- threegramsetfreq(dataset = words_news[1:1000])
head(freqthreegrams_news)
```
