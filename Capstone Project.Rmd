...
##### 1-Grams
Selected 1-grams dataset, using 20.000 strings out of the testdata dataset, with an accumulated frequency threshold (cutperc) of 80%:
```{r, cache = TRUE}
...
##### 2-grams
Applying almost the same "recipe" (frequency threshold "cutperc" raised to 90% due to fastest processing time), the selected 2-grams dataset:
```{r, cache = TRUE}
popular2grams <- ngramsetfreq(ngram = 2, dataset = sample(testdata, 20000)) %>%
        stringselect(., .9)
```

Due to fastest processing time, dataset size was raised to 1%
```{r, cache = TRUE}
wordpred2grams <- vectwordpred(popular2grams$string, 10, testdata, 0.01)
```
...
```{r, cache = TRUE}
wordpred3grams <- vectwordpred(popular3grams$string, 10, testdata, 0.01)
```

Further fine tunning can be carried on by modifying either the size of the dataset when applying ngramsetfreq function (currently at 90%), or the % of testdata ("sampdata" parameter) when applying the vectwordpred function.  Major consideration: processing time.
