...
```{r}
head(stringset("to the", final_news[1:1000]), 5)
```
...
```{r}
head(stringsure(string = "the faith during the", dataset = final_news), 5)
```

PENDIENTES:
1 VARIABILIZAR ESPACIO MUESTRAL EN FUNCIÓN DE N-GRAMAS A BUSCAR (A MAYOR N, MAYOR ESPACIO MUESTRAL).  O FIJAR UN N MÁXIMO, TRAS EL CUAL REBAJE STRING A DICHO N.
2 CONSTRUIR WRAPPER QUE ARROJE PREDICCIÓN DE SIGUIENTE PALABRA (DE ALGORITMOS ANTERIORES)
