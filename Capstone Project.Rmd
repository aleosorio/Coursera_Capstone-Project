...
varsum <- function(inputvect) {
        ...
        accsum <- 0
        for(i in 1:numrow) {
                accsum <- accsum + inputvect[i]
                result <- c(result, accsum)
        }
        return(result)
}
```
...
stringselect <- function(inputdf, cutperc) {
        rownum <- varselect(inputdf$freq, cutperc)
        freqvect <- unique(inputdf$freq[1:rownum])
        result <- filter(inputdf, freq %in% freqvect)
        return(result)
        
}
```

As an example, if applied to the 1-gram frequency twitter dataset previously obtained (in II.b.), with a 60% threshold, the following "tail" was obtained:
```{r}
tail(stringselect(freq_twitter, .6))
```
...
So, besides string N°4 ("a", whose next-word predictions vary between 4 possible values: "few", "little", "great" and "lot") the rest had pretty stable predictions.
...
As an example, when applied to a sring vector (it returns one or more solutions):
```{r}
freqvals(c("hello", "bye", "one", "hello", "bye", "two"))
```


##### Wrapper that converts input and output of vectwordpred function into a DF with input-string vector, word-predicted vector and solutions-per-string vector
...
Again, the "a" string obtains 4 possible solutions, out of which "few" was selected.

#### Obtained Results

##### 1-Grams
Selected 1-grams dataset, using 10.000 strings out of the testdata dataset, with an accumulated frequency threshold (cutperc) of 70%:
```{r, cache = TRUE}
popular1grams <- ngramsetfreq(ngram = 1, dataset = sample(testdata, 10000)) %>%
        stringselect(., .7)
```

Obtained input dataset:
```{r}
glimpse(popular1grams)
```

Applying vectwordpred function to the obtained input, with 10 predicted next words, within a dataset 0.5% of testdata:
```{r, cache = TRUE}
wordpred1grams <- vectwordpred(popular1grams$string, 10, testdata, 0.005)
```

The following matrix with 10 predictions for each 1-gram was obtained:
```{r}
head(wordpred1grams, 5)
```

Further fine tunning, basically can be made by increasing the % of testdata ("sampdata"" parameter) when applying the vectwordpred function.  Major consideration: processing time.

##### 2-grams
Applying same "recipe", the selected 2-grams dataset:
```{r, cache = TRUE}
