...
Further fine tunning can be carried on by modifying either the size of the dataset when applying stringselect function (currently between 85 and 90%), or the % of testdata ("sampdata" parameter) when applying the vectwordpred function (currently between 0.5 and 1%).  Major consideration: processing time.

#### Final Dataset
The final dataset from which popular-n-gram predictions were obtained:
```{r}
wordpredgrams <- 
```


### IV.b. Less Popular N-grams

#### Required Functions

##### Function that returns the position of the searched string, within the final popular-n-gram dataset

DEFINIR FORMATO DE WORDPREDGRAMS MÁS LIGERO POSIBLE (STRING VECTORS, DE BÚSQUEDA Y OUTPUT) Y ESTA FUNCIÓN, QUE HAGA BÚSQUEDA EXACTA DEL STRING DE BÚSQUEDA (Y NO CONSIDERE STRINGS TRUNCOS. IE: "TO THE" NO ARROJE "TO" | "THE" | "T" | "TO THE" | ETC.)
